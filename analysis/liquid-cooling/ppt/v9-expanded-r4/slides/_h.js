...
// ── Bullet list with colored bars ──
function bulletList(s, items, startY) {
  const yy = startY || 0.65;
  items.forEach((it, i) => {
    const y = yy + i * 0.78;
    s.addShape('rect', { x: 0.5, y, w: 0.05, h: 0.62, fill: { color: it.color || t.highlight } });
    s.addText(it.title, { x: 0.8, y, w: 8.5, h: 0.25, fontSize: 11, fontFace: 'Microsoft YaHei', color: it.color || t.primary, bold: true });
    s.addText(it.text, { x: 0.8, y: y + 0.25, w: 8.5, h: 0.37, fontSize: 9, fontFace: 'Microsoft YaHei', color: t.secondary });
  });
}
